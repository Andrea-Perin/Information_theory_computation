...
!!$  --------------------------------------------------------
  ...
  SUBROUTINE DGET_EGV(matr,egvals,info)
!!$  --------------------------------------------------------
!!$  This subroutine is a wrapper for the diagonalization
!!$  subroutine DSYEV provided by LAPACK. 
!!$  ARGUMENTS:
!!$  (I/O) matr : a REAL*8 matrix, the matrixto be diagonalized.
!!$               On return, it contains the eigenvectors.   
!!$  (O) egvals : a REAL*8 array, contains the eigenvalues
!!$  (O) info   : an INTEGER*4, communicating the status of
!!$               the diagonalization. If =0, all good.
!!$  --------------------------------------------------------
    ...
    ALLOCATE(WORK(1))
    ! querying optimal workspace
    CALL DSYEV('V','U',N,matr,N,egvals,WORK,LWORK,info)
    ! preparing to get the results
    LWORK=INT(WORK(1))
    DEALLOCATE(WORK)
    ALLOCATE(WORK(LWORK))
    ...
    ! removing exhausted variables 
    DEALLOCATE(WORK)
  END SUBROUTINE DGET_EGV


  FUNCTION DKRONECKER(mat1,mat2)
!!$  --------------------------------------------------------
!!$    This func computes the matrix-matrix Kronecker product,
!!$    the matrix representation of the tensor product, between
!!$    two real*8 matrices.
!!$    ARGUMENTS
!!$    - mat1        : a REAL*8 matrix, shape (a x b)    
!!$    - mat2        : a REAL*8 matrix, shape (c x d)
!!$    RETURNS
!!$    - dkronecker  : a REAL*8 matrix, shape (ac x bd)
!!$  --------------------------------------------------------
    REAL*8, DIMENSION(:,:) :: mat1,mat2
    REAL*8, DIMENSION(SIZE(mat1,1)*SIZE(mat2,1),&
         SIZE(mat1,2)*SIZE(mat2,2)) :: dkronecker
    INTEGER*4 :: ii,jj,rr,cc
    ...
    DO ii=1,SIZE(mat1,1)
       DO jj=1,SIZE(mat1,2)
          dkronecker((ii-1)*rr+1:ii*rr,(jj-1)*cc+1:jj*cc)=&
               mat1(ii,jj)*mat2
       END DO
    END DO
    RETURN
  ...
  FUNCTION PROJECT(proj,matr)
    REAL*8, DIMENSION(:,:) :: proj, matr
    REAL*8, DIMENSION(SIZE(proj,2),SIZE(proj,2)) :: project

    project=MATMUL(TRANSPOSE(proj),MATMUL(matr,proj))
  ...
  FUNCTION DEYE(dd)
!!$  --------------------------------------------------------
!!$    This subroutine creates a REAL*8 d-identity matrix.
!!$    ARGUMENTS
!!$    - dd    : an INTEGER*4, the dimension of the identity
!!$    RETURNS
!!$    - deye  : a REAL*8 matrix, shape dd x dd
!!$  --------------------------------------------------------
    INTEGER*4 :: dd,ii
    REAL*8, DIMENSION(dd,dd) :: deye
  ...
  FUNCTION DOUTER_PROD(vec1,vec2)RESULT(out)
!!$  --------------------------------------------------------
!!$    This func computes the double precision outer product of 
!!$    two given double real vectors. Copied by Numerical Recipes
!!$    in Fortran 90.
!!$    ARGUMENTS
!!$    - vec1    : a REAL*8 vector, size a    
!!$    - vec2    : a REAL*8 vector, size b
!!$    RETURNS
!!$    - out     : a REAL*8 matrix, shape (a x b)
!!$  --------------------------------------------------------
    REAL*8, DIMENSION(:) :: vec1,vec2
    REAL*8, DIMENSION(SIZE(vec1),SIZE(vec2)) :: out
    INTEGER*4 :: ii,jj
    out=SPREAD(vec1,dim=2,ncopies=SIZE(vec2))*&
         SPREAD(vec2,dim=1,ncopies=SIZE(vec1))
    RETURN
  END FUNCTION DOUTER_PROD


  FUNCTION DPARTIAL_TRACE(matr,lr,dd)RESULT(ptrace)
    REAL*8, DIMENSION(:,:) :: matr
    REAL*8, DIMENSION(SIZE(matr,1)/dd,SIZE(matr,2)/dd) :: ptrace
    INTEGER*4 :: dd, ii, jj, kk
    CHARACTER :: lr

    ptrace=0d0
    IF(lr.EQ.'R'.OR.lr.EQ.'r')THEN
       DO kk=0,dd-1
          ptrace=ptrace+matr(1+kk*dd:1+(kk+1)*dd,1+kk*dd:1+(kk+1)*dd)
       END DO
    END IF 
    IF(lr.EQ.'L'.OR.lr.EQ.'l')THEN
       DO ii=1,SIZE(ptrace,1)
          DO jj=1,SIZE(ptrace,2)
             ptrace(ii,jj)=DTRACE(matr((ii-1)*dd+1:ii*dd+1,&
                  (jj-1)*dd+1:jj*dd+1))
          END DO
       END DO
    END IF 
    IF(lr.NE.'L'.AND.lr.NE.'l'.AND.lr.NE.'R'.AND.lr.NE.'r')THEN
       PRINT*, "Invalid choice of subsystem."
    END IF
    RETURN
  END FUNCTION DPARTIAL_TRACE
...
!!$    eral square matrix.
!!$    ARGUMENTS
!!$    - mat    : a REAL*8 square matrix
!!$    RETURNS
!!$    - dtrace   : a REAL*8 number
!!$  --------------------------------------------------------

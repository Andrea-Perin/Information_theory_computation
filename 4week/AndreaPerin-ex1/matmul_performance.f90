...
CONTAINS
  ...
  SUBROUTINE READ_DIMENSION(file,sizes)
    ...
    ! program closes.
    ! ARGUMENTS
    ! - file     : a string, the name of the file containing the sizes
    ! - sizes    : an integer array, dim=2, containing the sizes read
  ...
  FUNCTION MATMUL_IJK(matrix1,matrix2)
    ...
    ! RETURNS:
    ! - matmul_ijk       : a REAL*8 matrix of shape (n,l). If the
    !                      input matrices are not compatible,
    !                      it is filled with zeros and a warning
  ...
  FUNCTION MATMUL_JKI(matrix1,matrix2)
    ...
    !                      no computation is carried out.
    ! RETURNS:
    ! - matmul_jki       : a REAL*8 matrix of shape (n,l). If the
    !                      input matrices are not compatible,
    !                      it is filled with zeros and a warning
    !                      is printed on screen.
    !-----------------------------------------------------------------
  ...
  FUNCTION MATMUL_KJI(matrix1,matrix2)
    ...
    !                      no computation is carried out.
    ! RETURNS:
    ! - matmul_kji       : a REAL*8 matrix of shape (n,l). If the
    !                      input matrices are not compatible,
    !                      it is filled with zeros and a warning
    !                      is printed on screen.
    !-----------------------------------------------------------------
  ...
  SUBROUTINE PRINT_RESULTS(resu,filename)
    !-----------------------------------------------------------
    ! This subroutine prints two numbers on an external file:
    ! first the size of the square matrix, and then the timing
    ! of the matrix-matrix multiplication algorithm.
    ! If the file exists, it is created; otherwise, the
    ! number is appended at the end of the file.
    ! ARGUMENTS:
    ! - resu      : a REAL*8, the result of the timing operation
    ! - filename  : the name of the file on which to print resu
    !-----------------------------------------------------------
    REAL*8 :: resu
    ...
    ELSE
       OPEN(15, file=filename, status="new", action="write")
    END IF
    WRITE(15, *) rows1, resu
    CLOSE(15)    
...
!------------------
  ...
  IF (CHECK_DIMS( (/rows1,cols1/) ,(/rows2,cols2/) )) THEN
     ...
     CALL PRINT_RESULTS(finish-start,"row.txt")
     ...
     CALL CPU_TIME(finish)
     CALL PRINT_RESULTS(finish-start,"col1.txt")
     ...
     CALL CPU_TIME(finish)
     CALL PRINT_RESULTS(finish-start,"col2.txt")
     ...
     CALL CPU_TIME(finish)
     CALL PRINT_RESULTS(finish-start,"matmul.txt")

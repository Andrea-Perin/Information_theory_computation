...
!!$  --------------------------------------------------------
  ...
  !DIAGONALIZING
!!$  --------------------------------------------------------  
  CALL GET_K_EGV(hamiltonian,kk,egvals,egvecs,info)
  CALL DEBUG(deb,info,info.NE.0,"ERROR: diagonalization failed.")
  PRINT*, lambda, egvals(:kk)/REAL(NN)
!!$  --------------------------------------------------------

  
!!$  !DENSITY MATRIX OF THE GROUND STATE
!  --------------------------------------------------------  
!!$  DEALLOCATE(hamiltonian,diag)
!!$  ALLOCATE(ground_state(2**NN))
!!$  ground_state=egvecs(:,1)
!!$  DEALLOCATE(egvecs)
!!$  ALLOCATE(dmat(2**NN,2**NN))
!!$  dmat=ZOUTER_PROD(ground_state,ground_state)
!!$  DO ii=1,NN-
!!$     dmat=TRACE_OUT(2,dmat,2,NN)
!!$     print*, "done"
!!$  END DO
!!$  PRINT*, dmat(:2,:2)
!  --------------------------------------------------------
